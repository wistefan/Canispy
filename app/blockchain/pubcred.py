...
if settings.PRODUCTION:
    DATABASE_FILE = "pubcred.sqlite"
else:
    DATABASE_FILE = "pubcred.test.sqlite"

DATABASE_NAME = os.path.join(settings.DATABASE_DIR, DATABASE_FILE)

table_schema = """
DROP TABLE IF EXISTS pubcred;

CREATE TABLE pubcred (
...
def new_certificate(uuid: str, certificate: str) -> str:
    ...
    db.execute(
        'REPLACE INTO pubcred (uuid, cert) VALUES (?, ?)',
        (uuid, certificate)
    )
